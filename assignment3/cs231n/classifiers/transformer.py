...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #2)
        tgt_mask = torch.tril(torch.ones(T, T, device=cap_embed.device, dtype=cap_embed.dtype))

        #3)
        features = self.transformer(tgt=cap_embed, memory=features_proj, tgt_mask=tgt_mask)

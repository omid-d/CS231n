...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            hidden_rnn_or_lstm, cache_rnn_or_lstm = rnn_forward(captions_in_init, hidden_init, Wx, Wh, b)

        else:
            hidden_rnn_or_lstm, cache_rnn_or_lstm = lstm_forward(captions_in_init, hidden_init, Wx, Wh, b)

      
        scores, cache_scores = temporal_affine_forward(hidden_rnn_or_lstm, W_vocab, b_vocab)
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask)


        #backward pass
        dhidden_rnn_or_lstm, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, cache_scores)

        if self.cell_type == 'rnn':
            dcaptions_in_init, dhidden_init, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dhidden_rnn_or_lstm, cache_rnn_or_lstm)

        else:
            dcaptions_in_init, dhidden_init, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dhidden_rnn_or_lstm, cache_rnn_or_lstm)

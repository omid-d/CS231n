...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = self.key(key).view(N, -1, self.n_head, self.head_dim)
        query = self.query(query).view(N, -1, self.n_head, self.head_dim)
        value = self.value(value).view(N, -1, self.n_head, self.head_dim)
        ...
        concat = scores.transpose(1,2).contiguous().view(N, -1, self.emd_dim)
